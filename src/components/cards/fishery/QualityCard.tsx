...
const QualityCard = () => {
  ...
  useEffect(() => {
    setDateOffset(0);
  }, [selectedTimeRange, selectedQuality]);
    ...
    if (chartRef.current) {
      ...
      if (ctx) {
        ...
        if (selectedTimeRange === "Weekly") {
          ...
          xAxisTitleText = `Week: ${format(viewStartDate, "MMM d")} - ${format(
            viewEndDate,
            "MMM d, yyyy"
          )}`;
        } else if (selectedTimeRange === "Monthly") {
        ...
        intervalDates.forEach((day) => {
          ...
          if (dataPoint && dataPoint.values[selectedQuality] !== undefined) {
            // For current or past dates, use the data or null if it was generated as null
            if (
              day <= today ||
              ...
              dataValues.push(null); // Future dates in a past month view, or beyond 'today' generally
            }
          } else if (
            day > today &&
            selectedTimeRange === "Monthly" &&
            isSameMonth(day, addMonths(referenceDate, dateOffset))
          ) {
        ...
        if (selectedTimeRange === "3 Months" && chartType === "line") {
        ...
        const chartTitleText = `${selectedQuality} (${selectedUnit}) - ${selectedTimeRange}`;
  ...
  }, [selectedQuality, selectedTimeRange, selectedUnit, dateOffset]);
    ...
    <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-lg shadow-sm">
      <div className="mb-4">
        ...
        <p className="text-sm text-dark dark:text-light mb-4 text-center sm:text-left">
          Select a parameter, unit, and time range to view trends.
        </p>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-end sm:items-center gap-3 sm:gap-4 mb-4">
          ...
          <div className="w-full sm:w-auto sm:min-w-[180px]">
            <DropdownSmall
              ...
              onSelect={(item) => setSelectedTimeRange(item as TimeRange)}
