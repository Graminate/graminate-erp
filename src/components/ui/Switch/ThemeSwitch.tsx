...
const ThemeSwitch = ({ switchAction }: Props) => {
  ...
  return (
    <label className="inline-flex flex-col cursor-pointer">
      ...
      />
      <div
        className="relative w-14 h-6 bg-gray-500 peer-focus:outline-none rounded-full 
      ...
      >
        {/* Moon Icon (Dark Mode) */}
        <span className="absolute left-2 top-1/2 -translate-y-1/2 text-yellow-500 text-base mr-2">
          <svg
        ...
        {/* Sun Icon (Light Mode) */}
        <span className="absolute right-2 top-1/2 -translate-y-1/2 text-grey-200 text-base ml-2">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
